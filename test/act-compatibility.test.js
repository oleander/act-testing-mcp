...
test("act --list output format is parseable", (t) => {
  ...
  // Check if we have workflows directory
  const workflowsPath = join(PROJECT_ROOT, ".github/workflows");
  if (!existsSync(workflowsPath)) {
    t.log("No .github/workflows directory found - skipping test");
    t.pass("Skipped: no workflows to test");
    return;
  }
  ...
  if (!result.success) {
    ...
    // For standalone repo without workflows, this is expected
    t.pass("Act list failed as expected (no workflows)");
    return;
  }
